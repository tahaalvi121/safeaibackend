...
router.post('/upload', upload.single('file'), async (req, res) => {
    try {
        ...
        const { tenantId = 'demo-tenant', userId = 'demo-user' } = req.body;
...
// Anonymize document
router.post('/:documentId/anonymize', async (req, res) => {
    try {
        const { documentId } = req.params;
...
router.post('/:documentId/question', async (req, res) => {
    try {
        const { documentId } = req.params;
        const { question, llmProvider = 'openai' } = req.body; // Support for specifying LLM provider
