...
class BadgeService {
    ...
    async generateBadge(type, tenantName, options = {}) {
        ...
        return {
            ...
            embedCode: this.getEmbedCode(badgeId, type),
            verificationUrl: `${process.env.APP_URL || 'http://localhost:3000'}/verify/${badgeId}`
        };
    }
  ...
  <text x="${width / 2}" y="95" 
     ...
     */
    getEmbedCode(badgeId, type) {
        const baseUrl = process.env.APP_URL || 'http://localhost:3000';

        return `<!-- SafeAI ${type} Compliance Badge -->
<a href="${baseUrl}/verify/${badgeId}" target="_blank" rel="noopener">
  <img src="${baseUrl}/badges/badge_${badgeId}.png" 
       alt="${type} Compliant - Verified by SafeAI" 
       width="200" 
       height="100" />
</a>`;
    }
